...
# CSV檔案的路徑
csv_file_path = "禽流感確診及撲殺數.csv"
df = pd.read_csv(csv_file_path)

# 將資料寫入資料庫
table_name = "birdflu"  # 資料表名稱
df.to_sql(table_name, con=engine, if_exists="replace", index=False)
print("birdflu.csv檔案已上傳到資料庫")

# 從資料庫讀取資料
query = "SELECT * FROM birdflu"
...
plt.plot(df["month"], df["data"])

# 添加標題和標籤
plt.title("過去5年禽流感確診及撲殺")
plt.xlabel("年份")
plt.ylabel("撲殺數", rotation=0)
